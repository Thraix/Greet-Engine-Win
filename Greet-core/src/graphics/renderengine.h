...
namespace greet { namespace graphics {
	...
	private:
		static std::map<std::string, Layer<Renderable>*> m_renderer2ds;
		static std::map<std::string, Layer3D*> m_renderer3ds;

	public:
		static void add_layer2d(Layer<Renderable>* renderer, const std::string& name);
		static void add_layer3d(Layer3D* renderer, const std::string& name);
		static Layer<Renderable>* remove_layer2d(const std::string& name);
		static Layer3D* remove_layer3d(const std::string& name);
		static Layer<Renderable>* get_renderer2d(const std::string& name);
